function MSS_RunMyMSS_IROCrating(expt)
...
for k = 1:numDevices,
    ...
    if (strcmp(devices(k).transport,'USB') && strcmp(devices(k).usageName,'Keyboard') && strcmp(devices(k).product,'Apple Internal Keyboard / Trackpad')), %note: this is the name of my default device-- may need to be updated depending on your system
        ...
        break,
    elseif (strcmp(devices(k).transport,'USB') && strcmp(devices(k).usageName,'Keyboard')) && ~strcmp(devices(k).product,'Xkeys'),
        homeDevice=k;
...
%[w, rect] = Screen('OpenWindow',screenNumber, [], [0 0 1600 1200]) %Debugging screen
...
for j = 1:number_of_trials,
    ...
    if st(i) == 'm' %primary stimulus is a movie        
        ...
        if st(i) == 't' %primary stimulus is text
            ...
            displaycommand = sprintf('DrawFormattedText(w,stFile{i},''center'',ycenter+.6*ycenter,white);'); %default_wrap,hshift(i),vshift(i))
        ...
        elseif st(i) == 'p' %primary stimulus is a picture
                eval(bg_display); %present the background file while picture is presented
                Screen('Flip',w);
            if strcmp('fixation',tag{i})
                Screen('FillRect',w,[0 0 0])  % black for fixation
            elseif strcmp('choose',tag{i})
                Screen('FillRect',w,[255 215 0])  % yellow for choose.jpg
                Screen('FillRect',w,[0 0 0], rect -[-75 -75 75 75]);
            elseif strcmp('regulate',tag{i})
                Screen('FillRect',w,[255 26 0])  % red for regulate.jpg
                Screen('FillRect',w,[0 0 0], rect -[-75 -75 75 75]);
            elseif strcmp('look',tag{i})
                Screen('FillRect',w,[0 181 26])  % green for look.jpg
                Screen('FillRect',w,[0 0 0], rect -[-75 -75 75 75]);
            elseif strcmp('image_c',tag{i})
                Screen('FillRect',w,[255 215 0])  % yellow for choose images
                Screen('FillRect',w,[0 0 0], rect -[-75 -75 75 75]);
            elseif strcmp('image_r',tag{i})
                Screen('FillRect',w,[255 26 0])  % red for regulate images
                Screen('FillRect',w,[0 0 0], rect -[-75 -75 75 75]);
            elseif strcmp('image_l',tag{i})
                Screen('FillRect',w,[0 181 26])  % green for look images
                Screen('FillRect',w,[0 0 0], rect -[-75 -75 75 75]);
            elseif strcmp('rate',tag{i})
                Screen('FillRect',w,[0 0 0]); % black for rate
            elseif strcmp('report',tag{i})
                Screen('FillRect',w,[0 0 0])  % black for report
            end
